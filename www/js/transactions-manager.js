    ...
    if (totalsContainer) {
        ...
        totalsContainer.innerHTML = availableCurrencies
            .map(curr => {
                ...
                return `
                    <div ${onclick} style="position: relative; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 6px 10px; border-radius: 8px; font-weight: 600; white-space: nowrap; opacity: ${opacity}; filter: ${grayscale}; cursor: ${cursor}; border: ${border}; transition: all 0.3s ease; box-shadow: ${isSelected && isMultiCurrency ? '0 4px 12px rgba(102, 126, 234, 0.4)' : 'none'};">
                        ${checkmark}
